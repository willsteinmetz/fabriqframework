      ...
      switch ($path->modpage) {
        case 'module':
          if (FabriqModules::enabled($path->controller)) {
            ...
            Fabriq\Core\Routing::arg(0, $path->controller);
            Fabriq\Core\Routing::arg(1, $path->action);
            for ($i = 0; $i < count($extra); $i++) {
              Fabriq\Core\Routing::arg($i + 2, $extra[$i]);
            }
        ...
        case 'page': default:
          ...
          if ($path->extra != '') {
            ...
            if ($path->wildcard) {
              $p = explode('/', $_GET['q']);
              Fabriq\Core\Routing::arg(2, $p[$path->wildcard]);
            }
          }
          return true;
        break;
