  ...
  public function create() {
    ...
    if (FabriqModules::module('roles')->requiresPermission('administer users', $this->name)) {
      ...
      if (count($errors) == 0) {
        ...
        if ($_POST['emailUser']) {
          $url = Fabriq\Core\Config::get('url') . PathMap::build_path('users', 'login');
          $title = Fabriq\Core\Config::get('title');
          $message = <<<EMAIL
Hello {$user->display},

An account has been created for you on the {$title} website.
...
EMAIL;
          mail(
            $user->email,
            "An account has been created for you at {$title}",
            $message,
            'From: noreply@' . str_replace('http://', '', str_replace('https://', '', str_replace('www.', '', Fabriq\Core\Config::get('url'))))
          );
        }
        $msg = "User added";
        $success = true;
...
Your password for {$title} has been reset.
...
Your account has been created on the {$title} website.
