...
<form method="post" action="<?php echo PathMap::build_path('sitemenus', 'items', 'create', $menu->id); ?>">
	<div style="padding: 2px;">
		<label for="<?php echo $moduleName; ?>_itemName">
			<strong>Item name:</strong>
		</label>
		<input type="text" name="<?php echo $moduleName; ?>_itemName" size="35" maxlength="100"<?php if ($submitted) { echo " value=\"{$menuItem->itemName}\""; } ?> />
	</div>
	<div style="padding: 2px;">
		<label for="<?php echo $moduleName; ?>_path">
			<strong>Path:</strong>
		</label>
		<input type="text" name="<?php echo $moduleName; ?>_path" size="35" maxlength="100"<?php if ($submitted) { echo " value=\"{$menuItem->path}\""; } ?> />
	...
	<div style="padding: 2px;">
		...
		for ($i = -20; $i <= 20; $i++) {
			echo "\t\t\t<option value=\"{$i}\"";
			if ($submitted && ($i == $menuItem->weight)) {
		...
		<select name="<?php echo $moduleName; ?>_parentItem">
			...
			<?php
				for ($i = 0; $i < count($menu->items); $i++) {
					echo $menu->items[$i]->getItemSelectOption();
				}
			?>
		</select>
	</div>
	<div style="padding: 2px;">
		...
		<input type="checkbox" value="1" name="<?php echo $moduleName; ?>_newWindow" />
	</div>
	<div style="padding: 2px;">
...
<?php else: ?>
<p>Menu item "<?php echo $menuItem->itemName; ?>" has been added.</p>
<p><a href="<?php echo PathMap::build_path('sitemenus', 'items', 'index', $menu->id); ?>">Return to menu items list</a></p>
<?php
	endif;
endif;
