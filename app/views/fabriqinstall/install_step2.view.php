...
<form method="post" action="<?php PathMap::build_path('fabriqinstall', 'install', 2); ?>">
	...
	<label for="url">URL <span class="required-field">*</span>: </label><input type="text" id="url" name="url" size="50" tabindex="5" <?php if ($submitted) { echo ' value="' . $_POST['url'] . '"'; } else { echo ' value="http://' . $_SERVER['HTTP_HOST'] . '"'; } ?>><br />
	<label for="apppath">Application path <span class="required-field">*</span>: </label><input type="text" id="apppath" name="apppath" value="<?php echo ($submitted) ? $_POST['apppath'] : $appPath; ?>" tabindex="6" /><br />
	...
	<label for="templating">Enable templating </label><input type="checkbox" id="templating" name="templating" value="true" tabindex="7"<?php if (($submitted && (($_POST['templating'] == true) || ($_POST['templating'] == 'true'))) || (!$submitted)) { echo ' checked="checked"'; } ?> /><br />
